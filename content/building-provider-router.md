...
- [PyPI](https://pypi.org/project/provider-router/) · [npm](https://www.npmjs.com/package/provider-router) — same name, both registries
...
The first instinct was to write the fallback inline. The second — the useful
one — was noticing I had already written it somewhere else.
...
Ordered failover sounds trivial, and the ordering genuinely is — it's an array.
The part that isn't trivial is that **every provider says "I'm rate limited" in
...
A router can't parse those; it doesn't know the domain. Your adapter can. That
translation — provider dialect into one shared vocabulary — turned out to be
the actual content of the library. Everything else follows from having it.

So the library ships **no adapters**, deliberately. An adapter encodes *your*
...
measure and useless to the caller. So results are graded `OK` or `DEGRADED`, and
a degraded answer is *kept but not settled for* — held while the router keeps
looking for a good one, returned only if nothing better appears, and always
labelled.
...
scenarios as data — provider behaviour in, expected decisions out — read by
...
`AttributeError` from inside the router — a bug that reads as a *router* fault
and sends you to the wrong file entirely. I'd written both within an hour of
each other and had no idea they disagreed.
...
nothing to do with the market — and an "alert me on any drop" rule would have
emailed about a fare that never moved. Automatic failover would have made that
happen without anyone deciding to. The comparison is now suppressed across a
provider change.
...
refusal — this is a public endpoint with a usage policy, and making it say no
...
`BUDGET` — and the fact that once providers are forced to describe themselves in
it, a lot of previously invisible failure becomes something you can act on.
